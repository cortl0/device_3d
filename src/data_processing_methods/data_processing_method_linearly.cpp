...
void data_processing_method_linearly::set_inputs(bnn::architecture& b, u_word& offset, u_word length, float value,
                                                 float range_from, float range_to, std::string& s, bool verbose)
{
    for(u_word j = 0; j < length; j++)
    {
        b.set_input(offset++, get_bool(range_from, range_to, value, length, j));
